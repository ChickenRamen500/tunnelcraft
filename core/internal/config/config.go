...
// Supports both AWG2 (legacy numeric junk params) and AWG3 (string H1-H4,
// header protection key, content padding, timing fields).
type AmneziaConfigEntry struct {
        ...
        S2           uint32 `yaml:"s2"`
        S3           uint32 `yaml:"s3"`
        // H1-H4 are strings to support AWG2 numeric and AWG3 range expressions.
        H1           string `yaml:"h1"`
        H2           string `yaml:"h2"`
        H3           string `yaml:"h3"`
        H4           string `yaml:"h4"`
        // AWG3-specific fields.
        HeaderProtectionKey    string `yaml:"header_protection_key"`
        ContentPaddingAddition string `yaml:"content_padding_addition"`
        RekeyAfterTime        string `yaml:"rekey_after_time"`
        RekeyTimeout          string `yaml:"rekey_timeout"`
